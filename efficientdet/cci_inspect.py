...
class ModelInspector(object):
  ...
  def build_model(self, inputs: tf.Tensor) -> List[tf.Tensor]:
    ...
    cls_outputs, box_outputs = cci_inference.build_model(
        self.model_name,
        inputs,
        **self.model_config)
  ...
  def export_saved_model(self, **kwargs):
    ...
    driver = cci_inference.ServingDriver(
        self.model_name,
        self.ckpt_path,
  ...
  def inference_single_image(self, image_image_path, output_dir, **kwargs):
    driver = cci_inference.InferenceDriver(self.model_name, self.ckpt_path,
  ...
  def build_and_save_model(self):
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
      ...
      if self.ckpt_path:
        ...
        cci_inference.restore_ckpt(sess, self.ckpt_path,
      ...
      tf.io.write_graph(sess.graph_def, self.logdir, self.model_name + '_train.pb', as_text=False)
  ...
  def freeze_model(self) -> Tuple[Text, Text]:
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
      ...
      tf.io.write_graph(graphdef, self.logdir, self.model_name + '_frozen.pb', as_text=False)
